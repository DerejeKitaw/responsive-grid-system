...
Перейдём к флексбоксу и флоатам. Соображения по их применению схожи, так что вы
можете выбрать любой из них и двигаться далее. Я буду использовать флоаты,
потому что они проще и понятнее новичкам.

Если вы выбрали флексбокс, помните об отличиях от флоатов, которые нужно учесть.

## Шаг 2: Задайте свойство `box-sizing: border-box`

Свойство `box-sizing` задаёт блочную модель, используемую браузерами для расчета
свойств `width` и `height`. Выставляя свойству `box-sizing` значение
`border-box`, мы сильно упрощаем расчёт размеров колонок и отступов
...
![Свойство box-sizing и его влияние на расчет ширины][6]

Обычно я устанавливаю значение `border-box` для всех элементов на сайте, 
благодаря этому ширина и высота элементов везде рассчитывается консистентно
